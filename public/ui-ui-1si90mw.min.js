...
Component: j-Console
Version: 1
Updated: 2023-10-05T23:45:00.000Z
Author: Peter Širka


Component: j-Controls
Version: 1
Updated: 2023-01-25T10:02:00.000Z
Author: Peter Širka
...
Component: j-Flow
Version: 1
Updated: 2023-10-05T01:11:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-08-17T10:54:00.000Z
Author: Peter Širka


Component: j-InlineProperties
Version: 1
Updated: 2023-08-07T18:51:00.000Z
Author: Peter Širka
...
Component: j-Parts
Version: 1
Updated: 2023-06-15T14:04:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-04-11T12:03:07.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-08-15T09:16:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-09-02T22:52:02.000Z
Author: Peter Širka
...
Component: j-Wysiwyg
Version: 1
Updated: 2023-08-16T09:25:00.000Z
Author: Peter Širka
*/
...
.ui-console{position:fixed;left:0;right:0;bottom:0;transition:all 0.15s;transform:translate(0,150px);height:150px;z-index:32;border:5px solid rgba(100,100,100,0.1);border-bottom:0}.ui-console-visible{transform:translate(0,0)}.ui-console-tabs{height:26px;padding:0;background-color:rgba(230,230,230,0.8);border-radius:var(--radius) var(--radius) 0 0}.ui-console-tabs span{float:left;min-width:24px;height:26px;line-height:26px;text-align:center;cursor:pointer;color:gray;font-size:12px;padding:0 7px}.ui-console-tabs span i{margin-right:5px}.ui-console-tabs > div span:first-child{border-radius:var(--radius) 0 0 0}.ui-console-tabs > div{float:left}.ui-console-tabs span:hover{color:#000}.ui-console-selected{background-color:#FFF;color:#000 !important}.ui-console-output{height:120px;background-color:#FFF;overflow:auto;overflow-scrolling:touch}.ui-console-tabs > .ui-console-close{color:red !important;float:right;border-right:0;margin:2px 0 0 3px;height:20px;line-height:20px;padding:0 5px}.ui-console-tabs > .ui-console-clear{color:#777;float:right;border-right:0;border-right:1px solid #D0D0D0;margin:2px 0 0 3px;height:20px;line-height:20px;padding:0 5px}.ui-console-message{height:24px;font-size:12px;border-bottom:1px solid #F0F0F0;line-height:24px;padding:0 5px 0 6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-right:30px}.ui-console-message i{width:12px;margin-right:7px;text-align:center}.ui-console-error{color:red}.ui-console-warning{color:#E8A826}.ui-console-success{color:#8CC152}.ui-console-info{color:#3BAFDA}.ui-console-body{overflow:hidden}.ui-dark .ui-console{border:5px solid rgba(0,0,0,0.8)}.ui-dark .ui-console-tabs{background-color:rgba(0,0,0,0.8)}.ui-dark .ui-console-output{background-color:#222}.ui-dark .ui-console-selected{background-color:#222;color:#FFF !important}.ui-dark .ui-console-tabs > .ui-console-clear{color:#555;border-right:1px solid #202020}.ui-dark .ui-console-message{border-bottom-color:#303030}.ui-dark .ui-console-tabs span:hover{color:#FFF}
...
COMPONENT('console',function(self,config,cls){var cls2='.'+cls,etabs,source,elogs,current,clearbtn,ready=false;self.singleton();self.readonly();self.make=function(){self.aclass(cls+' hidden');self.append('<div class="{0}-body"><div class="{0}-tabs"><span class="{0}-close"><i class="ti ti-times"></i></span><span class="{0}-clear hidden"><i class="ti ti-trash"></i></span><div></div></div><div class="{0}-output"></div></div>'.format(cls));etabs=self.find(cls2+'-tabs > div');elogs=self.find(cls2+'-output');clearbtn=self.find(cls2+'-clear');self.event('click',cls2+'-tab',function(){var el=$(this);var id=el.attrd('id');self.show(id)});self.event('click',cls2+'-clear',function(){source[current].items=[];clearbtn.aclass('hidden');elogs.empty()});self.event('click',cls2+'-close',function(){self.set(false)});self.on('resize2 + resize',self.resize);self.resize()};self.resize=function(){elogs.css('width',WW+30)};self.render_tabs=function(){if(!source)return;var keys=Object.keys(source);var builder=[];for(var i=0;i<keys.length;i++){var item=source[keys[i]];if(!current)current=keys[i];var icon=self.icon(item.icon);builder.push(('<span title="{1}" data-id="{2}" class="'+cls+'-tab{3}"><i class="{0}"></i>{1}</span>').format(icon+(item.name?'':'" style="margin-right:0'),item.name,keys[i],current===keys[i]?(' '+cls+'-selected'):''))}etabs.html(builder.join(''));current&&self.render_logs(source[current])};self.render_logs=function(obj){if(!obj){elogs.empty();return}var builder=[],arr=obj.items||EMPTYARRAY;for(var i=0;i<arr.length;i++){var item=arr[i],type=item.type||'info',icon=type==='error'?'bug':type==='warning'?type:type==='success'?'check-circle':'info-circle';builder.push('<div class="{0}-message {0}-{2}"><i class="ti ti-{3}"></i>{1}</div>'.format(cls,obj.encode===true||obj.encode==null?Thelpers.encode(item.body):item.body,type,icon))}elogs.html(builder.join(''));elogs[0].scrollTop=0;clearbtn.tclass('hidden',!builder.length)};self.show=function(id){if(current===id||!ready)return;etabs.find(cls2+'-selected').rclass(cls+'-selected');etabs.find(cls2+'-tab[data-id="{0}"]'.format(id)).aclass(cls+'-selected');current=id;self.render_logs(source[id])};self.rebind=function(path,value){if(!ready)return;source=value;if(path===config.datasource)self.render_tabs();else if(path.substring(config.datasource.length+1).substring(0,current.length)===current)self.render_logs(source[current])};self.configure=function(key,value){if(key==='datasource')self.datasource(value,self.rebind)};self.setter=function(value){if(value&&!ready){ready=true;self.rebind(config.datasource,GET(config.datasource))}if(value){self.rclass('hidden');if(typeof(value)==='string')self.show(value);self.aclass(cls+'-visible',100)}else{self.rclass('hidden',100);self.rclass(cls+'-visible')}}});
...
COMPONENT('parts','parent:auto;margin:0',function(self,config,cls){var skip=false,partw,parth,prev=[];self.make=function(){self.aclass(cls);self.on('resize + resize2',self.resize)};var itemreplace=function(item,content){return ADAPT(item.path,item.id,content)};var itempath=function(item,path){return path.replace(/\?/g,item.scope||item.path||item.id||'?')};var itemop=function(type,item,el){var fn=item[type];if(fn){if(typeof(fn)==='function')fn(item.element,item);else self.EXEC(itempath(item,fn),el||item.element,item)}};self.focus=function(id,fromsetter){var is=false,model=self.get();var classname=cls+'-focused',selected=model.findItem('id',id);if(fromsetter&&selected){if(selected.element.hclass(classname))return}var item=model.findItem('focused',true);if(item&&item.id!==id){is=true;item.focused=false;item.element.rclass(classname);itemop('blur',item);prev.push(item.id)}item=selected;if(item){if(!item.element.hclass(classname)){item.focused=true;item.element.aclass(classname);is=true}itemop('reload',item);itemop('focus',item)}if(is){skip=true;self.update(true)}};self.rename=function(id,name,icon){var model=self.get();var item=model.findItem('id',id);if(item){if(name)item.name=name;if(icon)item.icon=icon;skip=true;self.update(true)}};self.close=function(id){var model=self.get();if(id==null){prev=[];for(var item of model)self.close(item.id);return}var item=model.findItem('id',id);if(item){prev=prev.remove(id);var index=model.indexOf(item);model.splice(index,1);if(item.focused){var previd=prev.last();if(previd){var tmp=model.findIndex('id',previd);if(tmp!==-1)index=tmp}var next=model[index];if(!next)next=model[0];next&&setTimeout(self.focus,5,next.id)}skip=true;self.update(true);itemop('remove',item);item.element.remove();setTimeout2(self.ID+'free',FREE,1000)}};self.create=function(item){if(item.processed)return;item.processed=true;var div=$('<div></div>');div.aclass(cls+'-item invisible');div.attrd('id',item.id);if(item.attr){for(var key in item.attr)div.attr(key,item.attr[key])}if(item.attrd){for(var key in item.attrd)div.attrd(key,item.attrd[key])}if(parth||partw)div.css({width:partw,height:parth});if(item.import){IMPORT(item.import,div,function(){itemop('init',item,div);setTimeout2(self.ID+'focus',self.focus,100,null,item.id);div.rclass('invisible',item.delay||10);item.import=null},true,function(content){return itemreplace(item,content)})}else if(div.html){div.append(itemreplace(item,item.html));item.html.COMPILABLE()&&setTimeout(COMPILE,1,div)}div[0].$part=item;item.element=div;itemop('create',item);self.append(div);if(!item.import){setTimeout2(self.ID+'focus',self.focus,100,null,item.id);div.rclass('invisible',item.delay||10);if(item.init){if(typeof(item.init)==='function')item.init(div,item);else self.EXEC(true,itempath(item,item.init),div,item)}}};self.resize=function(){setTimeout2(self.ID,self.resizeforce,300)};self.resizeforce=function(){var parent=self.parent(config.parent);var w=parent.width();var h=parent.height()-config.margin;if(parth!==h||partw!==w){parth=h;partw=w;self.find('> div').css({width:w,height:h});self.element.SETTER('*/resize')}};self.setter=function(value){if(skip){skip=false;return}var model=value||EMPTYARRAY;for(var item of model){self.create(item);item.focused&&self.focus(item.id)}var div=self.find('> div').toArray();for(var i=0;i<div.length;i++){var el=$(div[i]);if(!model.findItem('id',el.attrd('id'))){var obj=div[i].$part;if(obj){itemop('remove',obj);obj.element.remove()}}}self.resize()}});
